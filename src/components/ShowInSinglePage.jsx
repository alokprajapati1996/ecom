...
const ShowInSinglePage = () => {
  ...
  <div className="lg:max-w-6xl max-w-xl  mx-auto">
    <div className="grid  items-start grid-cols-1 lg:grid-cols-2 gap-8
     max-lg:gap-12 max-sm:gap-8">
      <div className="w-full lg:sticky top-0">
        <div className="flex  flex-row gap-2">
          {/* side images */}
          { location.state.images.map((obj,i)=>(
          <div className="flex flex-col gap-2 w-16 h-max  max-sm:w-14 shrink-0">
            <img key={i} src={obj} alt="Product1" className="aspect-[64/85] object-cover object-top w-full cursor-pointer  border-b-2 border-black" />
            </div>
          ))
        }
          {/* full images */}
          <div className="flex-1 border-2 ">
            <img src={location.state.thumbnail} alt="Product" className="
            w-full  aspect-[348/412] object-contain" />
          </div>
        </div>
      </div>
      <div className="w-full">
        <div>
          <h3 className="text-lg sm:text-xl font-bold text-gray-800">Women Embroidered A-line Kurta</h3>
          <p className="text-gray-500 mt-1 text-sm">Women Embroidered Georgette A-line Kurta With Attached Dupatta (Maroon)
          </p>
          <div className="flex items-center flex-wrap gap-4 mt-4">
            <h4 className="text-gray-800 text-2xl sm:text-3xl font-bold">${location.state.price}</h4>
          ...
          <div className="flex items-center gap-4 mt-2">
            <div className="flex items-center gap-1 text-lg px-2.5 bg-green-600 text-white rounded-full">
              ...
              <svg className="w-[13px] h-[13px] fill-white" viewBox="0 0 14 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M7 0L9.4687 3.60213L13.6574 4.83688L10.9944 8.29787L11.1145 12.6631L7 11.2L2.8855 12.6631L3.00556
                 8.29787L0.342604 4.83688L4.5313 3.60213L7 0Z" />
              </svg>
            </div>
        ...
        <div>
          ...
          <div className="mt-6 flex flex-wrap gap-4">
            <button type="button" className="px-4 py-3 w-[45%] border border-gray-300 bg-gray-100 hover:bg-gray-200
             text-gray-800 text-sm font-semibold  ">Add
              to wishlist</button>
            <button type="button" className="px-4 py-3 w-[45%] border border-blue-600 bg-blue-600 hover:bg-blue-700
             text-white text-sm font-semibold  " onClick={()=>dispatch(addToCart(location))}>Add
              to cart</button>
          </div>
        </div>
        <hr className="my-6 border-gray-300" />
          ...
          <div className="mt-2" role="accordion">
            <div className="hover:bg-gray-100 transition-all">
              ...
              <div className="pb-4 px-4">
                <p className="text-sm text-gray-500 leading-relaxed">{location.state.description}</p>
              </div>
            </div>
            <div className="hover:bg-gray-100 transition-all">
              <button type="button" className="w-full text-sm font-semibold text-left px-4 py-2.5 text-gray-800 flex items-center">
            ...
            <div className="hover:bg-gray-100 transition-all">
              <button type="button" className="w-full text-sm font-semibold text-left px-4 py-2.5 text-gray-800 flex 
              items-center" onClick={()=>setIsVisible((prev)=>!prev)}>
                <span className="mr-4">Return and exchange policy</span>
                <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 fill-current ml-auto shrink-0 -rotate-90" viewBox="0 0 24 24">
                  <path fillRule="evenodd" d="M11.99997 18.1669a2.38 2.38 0 0 1-1.68266-.69733l-9.52-9.52a2.38 2.38 0 1 1 3.36532-3.36532l7.83734 7.83734 7.83734-7.83734a2.38 2.38 0 1 1 3.36532 3.36532l-9.52 9.52a2.38 2.38 0 0 1-1.68266.69734z" clipRule="evenodd" data-original="#000000" />
                </svg>
              </button>
              {isVisible && <div className="pb-4 px-4">
                <p className="text-sm text-gray-500 leading-relaxed">Lorem ipsum dolor sit amet, consectetur adipiscing
                  elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
              </div>}
            </div>
        ...
        <div>
          ...
          <div className="flex items-center flex-wrap gap-4 mt-4">
            <h4 className="text-2xl sm:text-3xl text-gray-800 font-semibold">{location.state.rating} / 5</h4>
            <p className="text-sm text-gray-500">Based on 253 ratings</p>
          </div>
        </div>

        <div className="mt-6">
          {location.state.reviews.map((ele,i)=>{
          return <div className="flex items-start" key={i}>
            <img src="https://readymadeui.com/team-2.webp" className="w-12 h-12 rounded-full border-2 border-white" />
            <div className="ml-3">
              <h4 className="text-sm font-bold">{ele.reviewerName}  ({ele.reviewerEmail})</h4>
              ...
              {ele.rating}
                ...
                <p className="text-xs text-gray-500 !ml-2">{new Date(ele.date).toLocaleDateString()}</p>
              </div>
              <p className="text-sm text-gray-500 mt-4">{ele.comment}</p>
            </div>
...
export default ShowInSinglePage