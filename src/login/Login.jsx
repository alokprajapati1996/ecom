...
import { setLoggin } from '../features/Authentication';
...
const Login = () => {
  ...
  let dispatch=useDispatch()
   const handleLogin=(e)=>{
    e.preventDefault()
    let database=JSON.parse(localStorage.getItem("data_ecom"))||[]
    const obj={
    ...
    if(findUser){
      if(findUser.password===obj.password){
        ...
        dispatch(setLoggin({email:obj.email}))
       navigate('/')
      }else{
        alert('wrong password!')
      }
    }else{
      return alert('user not find please signup!')
    }
        ...
        <form>
          <div className="mb-12">
            ...
            <p className="text-sm mt-4 text-gray-800">
              Don't have an account <Link to={"/sign-up"} className="text-blue-600 font-semibold hover:underline ml-1 whitespace-nowrap">Register here</Link></p>
          </div>
          <div>
            <label className="text-gray-800 text-xs block mb-2">Email</label>
            <div className="relative flex items-center">
              <input name="email" type="text" required ref={emailRef} className="w-full text-gray-800 text-sm border-b border-gray-300 focus:border-blue-600 pl-2 pr-8 py-3 outline-none" placeholder="Enter email" />
          ...
          <div className="mt-8">
            ...
            <div className="relative flex items-center">
              <input name="password" type="password" ref={passwordRef} required className="w-full text-gray-800 text-sm border-b border-gray-300 focus:border-blue-600 pl-2 pr-8 py-3 outline-none" placeholder="Enter password" />
          ...
          <div className="mt-12">
            <button type="button" onClick={handleLogin} className="w-full shadow-xl py-2.5 px-4 text-sm tracking-wide rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none">
              Sign in
            </button>
          </div>
